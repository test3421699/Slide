...
async function startServer() {
  ...
  app.post("/api/generate-slides", async (req, res) => {
    try {
      ...
      if (!apiKey) {
        res.status(500).json({
          error: "GEMINI_API_KEY is not configured. Please set it in the Settings Secrets tab."
        });
        return;
      }
...
Let the background colors vary subtly between slides to create a rhythmic pacing, but ensure cohesive styling.`;
      ...
      res.status(500).json({ error: error.message || "Something went wrong on the slide gen server." });
    }
  });

  // Serve static dist in production, use Vite in dev
